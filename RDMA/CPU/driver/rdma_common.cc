...
void show_rdma_buffer_attr(struct rdma_buffer_attr *attr) {
  ...
  printf("---------------------------------------------------------\n");
  printf("buffer attr, addr: %p , len: %llu , stag : 0x%x \n",
         (void *)attr->address, (unsigned long long int)attr->length,
...
struct ibv_mr *rdma_buffer_alloc(struct ibv_pd *pd, uint64_t size,
  ...
  debug("Buffer allocated: %p , len: %llu \n", buf, size);
...
struct ibv_mr *rdma_buffer_register(struct ibv_pd *pd, void *addr,
  ...
  debug("Registered: %p , len: %llu , stag: 0x%x \n", mr->addr,
