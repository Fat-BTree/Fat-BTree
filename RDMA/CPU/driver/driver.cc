...
void dpdk_driver::send_pkt(char *context, unsigned nb_bytes, uint16_t send_no,
  ...
  hdr_udp = (struct rte_udp_hdr *)rte_pktmbuf_prepend(
      mbuf, sizeof(struct rte_udp_hdr));
  ...
  hdr_ip = (struct rte_ipv4_hdr *)rte_pktmbuf_prepend(
      mbuf, sizeof(struct rte_ipv4_hdr));
  hdr_ip->version_ihl = 0x45;
  hdr_ip->type_of_service = 0;
  hdr_ip->total_length = htons(nb_bytes + sizeof(struct rte_udp_hdr) +
  ...
  hdr_eth = (struct rte_ether_hdr *)rte_pktmbuf_prepend(
...
void dpdk_driver::recv_pkt(char *context, unsigned nb_bytes, uint16_t &recv_no,
  ...
  if (recv_queue[queue_id].empty()) {
    /* Send burst of TX packets, to second port of pair. */
    uint16_t nb_rx;

    /* Free any unsent packets. */
    do {
      nb_rx = rte_eth_rx_burst(port, queue_id, mbufs, 4);
    } while (unlikely(nb_rx < 1));

    for (int buf = 0; buf < nb_rx; buf++) {
      recv_queue[queue_id].push(mbufs[buf]);
    }
  }

  mbuf = recv_queue[queue_id].front();
  recv_queue[queue_id].pop();
...
unsigned inject_fbt_hdr(char *msg, uint64_t fbt_key) {
  ...
  memcpy(msg + msg_len, (char *)&fbt_idx, 6);
  msg_len += 6;
...
unsigned inject_key_index(char *msg, KEY_TYPE &key_index) {
  unsigned msg_len = 0;
  unsigned ubuf;

  ubuf = key_index.size();
  memmove(msg + msg_len, (char *)&ubuf, sizeof(unsigned));
  msg_len += sizeof(unsigned); // key_index_len

  memmove(msg + msg_len, key_index.begin(), key_index.size());
  msg_len += key_index.size(); // key_index

#ifdef DEBUG
  fprintf(stderr, "key_index length: %d; key_index: ", ubuf);
  for (int i = 0; i < ubuf; i++) {
    fprintf(stderr, "%c", key_index.begin()[i]);
  }
#endif
...
unsigned inject_values(char *msg, VAL_TYPE &values) {
  unsigned msg_len = 0;
  unsigned ubuf;
  ...
  for (size_t i = 0; i < values.size(); i++) {
    ubuf = values[i].first.size();
    memmove(msg + msg_len, (char *)&ubuf, sizeof(unsigned));
    msg_len += sizeof(unsigned); // values_k_len

    memmove(msg + msg_len, values[i].first.begin(), values[i].first.size());
    msg_len += values[i].first.size(); // values_k
    ...
    ubuf = values[i].second.size();
    memmove(msg + msg_len, (char *)&ubuf, sizeof(unsigned));
    msg_len += sizeof(unsigned); // values_v_len

    memmove(msg + msg_len, values[i].second.begin(), values[i].second.size());
    msg_len += values[i].second.size(); // values_v
...
unsigned extract_fbt_hdr(char *msg, int64_t &fbt_idx) {
  unsigned msg_len = 0;
  unsigned ubuf;
  int8_t in_idx[8] = {0};
...
unsigned extract_key_index(char *msg, KEY_TYPE &key_index) {
  unsigned msg_len = 0;
  unsigned ubuf;
  ...
  key_index.resize(ubuf);
  memcpy(key_index.begin(), msg + msg_len, ubuf);
  msg_len += key_index.size(); // key_index
...
unsigned extract_values(char *msg, VAL_TYPE &values) {
  unsigned msg_len = 0;
  unsigned ubuf;
  ...
  for (size_t i = 0; i < values.size(); i++) {
    ...
    values[i].first.resize(ubuf);
    memcpy(values[i].first.begin(), msg + msg_len, ubuf);
    msg_len += values[i].first.size(); // values_k
    ...
    values[i].second.resize(ubuf);
    memcpy(values[i].second.begin(), msg + msg_len, ubuf);
    msg_len += values[i].second.size(); // values_v
