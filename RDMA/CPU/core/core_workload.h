...
class CoreWorkload {
 public:
  ...
  virtual std::string NextTransactionKey(double MixtureRatio); /// Used for transactions
...
inline std::string CoreWorkload::NextTransactionKey(double mixture_proportion = 0.0) {
  uint64_t key_num;
  do {
    key_num = key_chooser_->Next();
  } while (key_num > insert_key_sequence_.Last());
  if (mixture_proportion > 0 && utils::RandomDouble() < mixture_proportion) {
    key_num = utils::Hash(key_num) % (insert_key_sequence_.Last() + 1);
  }
  return BuildKeyName(key_num);
}
