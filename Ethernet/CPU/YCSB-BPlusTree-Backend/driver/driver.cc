...
void dpdk_driver::send_pkt(char *context, unsigned nb_bytes, uint16_t send_no,
  ...
  hdr_udp = (struct udp_hdr *)rte_pktmbuf_prepend(
      mbuf, sizeof(struct udp_hdr));
  ...
  hdr_ip = (struct ipv4_hdr *)rte_pktmbuf_prepend(
      mbuf, sizeof(struct ipv4_hdr));
  hdr_ip->version_ihl = 0x45;
  hdr_ip->type_of_service = 0;
  hdr_ip->total_length = htons(nb_bytes + sizeof(struct udp_hdr) +
  ...
  hdr_eth = (struct ether_hdr *)rte_pktmbuf_prepend(
...
void dpdk_driver::recv_pkt(char *context, unsigned nb_bytes, uint16_t &recv_no,
  ...
  if (recv_queue[queue_id].empty()) {
    ...
    /* Free any unsent packets. */
    do {
      nb_rx = rte_eth_rx_burst(port, queue_id, mbufs, BURST_SIZE);
 ...
 * functions.
 */
int dpdk_driver::init(int argc, char *argv[], int nb_threads, uint16_t portid) {
  unsigned nb_ports;
  uint16_t q;
...
unsigned inject_msg_type(char *msg, unsigned msg_type, uint8_t pid) {
  unsigned msg_len = 0;
  unsigned ubuf;
  ...
  memcpy(msg + msg_len, (char *)&ubuf, sizeof(uint8_t));
...
unsigned extract_msg_type(char *msg, unsigned &msg_type) {
  ...
  ubuf = *(uint8_t *)(msg + msg_len);
  msg_len += sizeof(uint8_t); // Insert
