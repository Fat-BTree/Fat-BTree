...
public:
  ...
  int init(int argc, char *argv[], int nb_threads = 1, uint16_t portid = 0);
...
private:
  ...
  std::unordered_map<std::thread::id, uint16_t> map_queue_id;
  std::unordered_map<std::thread::id, uint16_t> map_dest_port;
  std::mutex mutex_;

  std::queue<rte_mbuf *> recv_queue[MAX_QUEUE_NUM];
};
